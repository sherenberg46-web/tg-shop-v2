      ...
      <div style={{ position: 'relative' }}>
        ...
        {pct > 0 && (
          <div style={{
            ...
            background: 'linear-gradient(135deg, #ff6b00, #c0001a)',
...
export default function BigGamesSale() {
  const nav       = useNavigate()
  const region    = useStore(s => s.region)
  ...
  useEffect(() => {
    api.get<Product[]>('/products', { params: { sort: 'discount', limit: 200 } })
      .then(r => {
        ...
        setGames(data.filter(p => (p.discount_pct ?? 0) > 0))
      })
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: 'linear-gradient(135deg, #1a0005 0%, #6b0010 35%, #c0001a 65%, #ff6b00 100%)',
        padding: 'max(10px, calc(var(--tg-top, 0px) + 6px)) 16px 12px',
        display: 'flex', alignItems: 'center', gap: 12,
        borderBottom: '1px solid rgba(255,107,0,.3)',
      }}>
        <button
          onClick={() => nav(-1)}
          style={{
        ...
        {!loading && (
          <span style={{
            ...
            color: '#ff9040', fontSize: 12, fontWeight: 700,
      ...
      <main style={{
            ...
            <button
              ...
              style={{
                ...
                background: sort === s.id
                  ? 'linear-gradient(135deg, #c0001a, #ff6b00)'
                  : 'var(--bg-card)',
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 60, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 17, fontWeight: 700, marginBottom: 6 }}>Распродажа скоро</p>
            <p style={{ fontSize: 14, color: 'var(--text-secondary)' }}>Пока нет товаров со скидкой</p>
          </div>
        ) : (
          <div className="game-grid-3">
